...
function global:Get-SMFModuleInfo {
    return [PSCustomObject]@{
        ...
        Version     = "1.0.0"
        Description = "Analyze and clear Microsoft Edge cache"
    }
}

function global:Invoke-ClearEdgeCache {
    param($Context)

    $Result = New-SMFResult -ModuleName "Clear-EdgeCache"

    try {
        $Settings = $Context.Config.Settings.BrowserCache.Edge
        if (-not $Settings.Enabled) {
            Add-SMFResultDetail $Result "Edge cache module disabled"
        ...
        foreach ($User in $Users) {
            ...
            $EdgeRoot = Join-Path $User.FullName "AppData\Local\Microsoft\Edge\User Data"
            if (-not (Test-Path $EdgeRoot)) { continue }
