...
function global:Get-SMFModuleInfo {
    return [PSCustomObject]@{
        Name        = "Clear-ChromeCache"
        Version     = "1.0.0"
        Description = "Analyze and clear Google Chrome cache"
    }
}

function global:Invoke-ClearChromeCache {
    param($Context)

    $Result = New-SMFResult -ModuleName "Clear-ChromeCache"

    try {
        $Settings = $Context.Config.Settings.BrowserCache.Chrome
        if (-not $Settings.Enabled) {
            Add-SMFResultDetail $Result "Chrome cache module disabled"
        ...
        foreach ($User in $Users) {
            ...
            $ChromeRoot = Join-Path $User.FullName "AppData\Local\Google\Chrome\User Data"
            if (-not (Test-Path $ChromeRoot)) { continue }
